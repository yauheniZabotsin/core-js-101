...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (closeBracket.indexOf(str[i]) !== -1) {
      if (arrStack.length === 0 || arrStack.pop() !== openBracket[closeBracket.indexOf(str[i])]) {
        return false;
      }
    } else {
      arrStack.push(str[i]);
...
function getCommonDirectoryPath(pathes) {
  ...
  while (i < array.length) {
    const directory = array[i];
    for (let j = 0; j < pathes.length; j += 1) {
      const arr = pathes[j].split('/');
      if (arr[i] !== directory) return strDir;
    }
    strDir += `${directory}/`;
    i += 1;
  }
